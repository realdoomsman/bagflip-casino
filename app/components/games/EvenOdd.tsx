      ...
      {!isRolling && !result && (
        <div className="grid grid-cols-2 gap-3 mb-6">
          <button onClick={() => setChoice('even')} className={`choice-btn ${choice === 'even' ? 'choice-btn-selected' : ''}`}>
            <div className="text-2xl sm:text-3xl font-bold text-white mb-1">2, 4, 6</div>
          ...
          <button onClick={() => setChoice('odd')} className={`choice-btn ${choice === 'odd' ? 'choice-btn-selected' : ''}`}>
            <div className="text-2xl sm:text-3xl font-bold text-white mb-1">1, 3, 5</div>
        ...
        <AnimatePresence mode="wait">
          {isRolling ? (
            <motion.div key="roll" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center">
              <motion.div className="text-5xl sm:text-6xl font-bold text-fuchsia-400" animate={{ scale: [1, 1.1, 1] }} transition={{ duration: 0.1, repeat: Infinity }}>
                {displayNum}
              </motion.div>
            ...
            <motion.div key="result" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
              ...
              <div className="text-sm text-white/40 mb-4">{result.number % 2 === 0 ? 'Even' : 'Odd'}</div>
            ...
            <motion.div key="wait" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center">
              <div className="text-5xl font-bold text-white/10 mb-3">?</div>
              <div className="text-sm text-white/30">Pick even or odd</div>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
