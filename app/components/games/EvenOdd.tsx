...
export default function EvenOdd() {
  ...
  return (
    <div className="p-6 rounded-xl border border-white/10 bg-white/[0.02]">
      ...
      {!isRolling && !result && (
        <div className="mb-5">
          <label className="block text-xs text-white/40 mb-2 uppercase tracking-wide">Pick Type</label>
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => setChoice('even')} className={`p-4 rounded-lg border-2 transition ${choice === 'even' ? 'border-[#ef4444] bg-[#ef4444]/5' : 'border-white/10 hover:border-white/20'}`}>
              <div className="text-2xl font-bold text-white">2, 4, 6</div>
              <div className="text-xs text-white/40">Even</div>
            </button>
            <button onClick={() => setChoice('odd')} className={`p-4 rounded-lg border-2 transition ${choice === 'odd' ? 'border-[#ef4444] bg-[#ef4444]/5' : 'border-white/10 hover:border-white/20'}`}>
              <div className="text-2xl font-bold text-white">1, 3, 5</div>
              <div className="text-xs text-white/40">Odd</div>
            </button>
          </div>
        </div>
      )}
      ...
      {!result && (
        <button onClick={handlePlay} disabled={!choice || !wager || isRolling || loading}
          ...
          {!connected ? 'Connect Wallet to Play' : isRolling ? 'Rolling...' : 'Roll Number'}
        </button>
      )}
    </div>
  )
