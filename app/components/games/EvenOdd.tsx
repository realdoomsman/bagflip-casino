...
export default function EvenOdd() {
  ...
  return (
    <div className="p-6 rounded-xl border border-white/10 bg-white/[0.02]">
      ...
      {!isRolling && !result && (
        <div className="mb-6">
          <label className="block text-sm text-white/50 mb-2">Pick Type</label>
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => setChoice('even')} className={`p-4 rounded-lg border transition ${choice === 'even' ? 'border-[#ef4444] bg-[#ef4444]/10' : 'border-white/10 hover:border-white/20'}`}>
              <div className="text-2xl font-bold text-white">2, 4, 6</div>
              <div className="text-sm text-white/50">Even</div>
            </button>
            <button onClick={() => setChoice('odd')} className={`p-4 rounded-lg border transition ${choice === 'odd' ? 'border-[#ef4444] bg-[#ef4444]/10' : 'border-white/10 hover:border-white/20'}`}>
        ...
        <AnimatePresence mode="wait">
          {isRolling ? (
            ...
            <div className="text-center text-white/20">
              ...
              <div className="text-sm mt-2">Pick a type</div>
            </div>
          )}
        </AnimatePresence>
      </div>
