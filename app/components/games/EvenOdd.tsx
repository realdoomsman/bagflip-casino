  ...
  return (
    <div className="bg-[#12121a] rounded-2xl border border-white/5 overflow-hidden">
      ...
      <div className="p-5">
        ...
        {!isRolling && !result && (
          <div className="mb-5">
            <label className="text-xs font-medium text-white/40 uppercase tracking-wide mb-3 block">Pick Type</label>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setChoice('even')} className={`p-5 rounded-xl border-2 transition-all ${choice === 'even' ? 'bg-[#ff6b4a]/10 border-[#ff6b4a]' : 'bg-[#1a1a24] border-white/5 hover:border-white/20'}`}>
                <div className="text-3xl font-black text-white mb-1">2, 4, 6</div>
                <div className="text-sm text-white/50">Even</div>
              </button>
              <button onClick={() => setChoice('odd')} className={`p-5 rounded-xl border-2 transition-all ${choice === 'odd' ? 'bg-[#ff6b4a]/10 border-[#ff6b4a]' : 'bg-[#1a1a24] border-white/5 hover:border-white/20'}`}>
