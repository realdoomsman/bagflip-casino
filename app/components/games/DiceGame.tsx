  ...
  return (
    <div className="bg-zinc-900 rounded-2xl border border-zinc-800 p-6">
      {!isRolling && !result && (
        <>
          <div className="mb-6">
            <label className="block text-sm text-zinc-400 mb-3">Pick a range</label>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setChoice('low')} className={`p-5 rounded-xl border-2 transition ${choice === 'low' ? 'border-white bg-white/5' : 'border-zinc-700 hover:border-zinc-600'}`}>
                <div className="text-2xl font-bold text-white">1-50</div>
                <div className="text-sm text-zinc-400">Low</div>
              </button>
              <button onClick={() => setChoice('high')} className={`p-5 rounded-xl border-2 transition ${choice === 'high' ? 'border-white bg-white/5' : 'border-zinc-700 hover:border-zinc-600'}`}>
