  ...
  return (
    <div className="bg-[#181818] rounded-2xl border border-white/[0.06] overflow-hidden">
      ...
      <div className="p-5">
        ...
        {!isRolling && !result && (
          <div className="mb-5">
            <label className="text-xs font-medium text-white/40 mb-2 block">PICK RANGE</label>
            <div className="grid grid-cols-2 gap-3">
              <button onClick={() => setChoice('low')} className={`p-5 rounded-xl border-2 transition-all ${choice === 'low' ? 'bg-blue-500/10 border-blue-500' : 'bg-[#0f0f0f] border-white/[0.06] hover:border-white/20'}`}>
                <div className="text-2xl font-bold text-white mb-1">1-50</div>
                <div className="text-sm text-white/50">Low</div>
              </button>
              <button onClick={() => setChoice('high')} className={`p-5 rounded-xl border-2 transition-all ${choice === 'high' ? 'bg-blue-500/10 border-blue-500' : 'bg-[#0f0f0f] border-white/[0.06] hover:border-white/20'}`}>
