    ...
    <div className="game-card">
      ...
      {!isRolling && !result && (
        <div className="grid grid-cols-2 gap-3 mb-6">
          <button onClick={() => setChoice('low')} className={`choice-btn ${choice === 'low' ? 'selected' : ''}`}>
            <div className="text-3xl font-bold text-white mb-1">1-50</div>
            <div className="text-sm text-white/50">Low</div>
          </button>
          <button onClick={() => setChoice('high')} className={`choice-btn ${choice === 'high' ? 'selected' : ''}`}>
            <div className="text-3xl font-bold text-white mb-1">51-100</div>
        ...
        <AnimatePresence mode="wait">
          {isRolling ? (
            ...
            <motion.div key="result" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
              ...
              <div className="text-sm text-white/40 mt-1 mb-5">{result.number > 50 ? 'High' : 'Low'}</div>
