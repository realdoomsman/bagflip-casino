    ...
    <div className="bg-[#111a1a] rounded-2xl border border-cyan-500/20 overflow-hidden glow-cyan">
      <div className="p-6">
        {!isRolling && !result && (
          <>
            <div className="mb-6">
              <label className="block text-xs text-white/40 mb-3 uppercase tracking-wider">Pick Range</label>
              <div className="grid grid-cols-2 gap-4">
                <button onClick={() => setChoice('low')} className={`p-6 rounded-xl border-2 transition-all ${choice === 'low' ? 'border-cyan-500 bg-cyan-500/10' : 'border-white/10 hover:border-white/20 bg-white/[0.02]'}`}>
                  <div className="text-3xl font-black text-white mb-1">1-50</div>
                  <div className="text-white/50">Low</div>
                </button>
                <button onClick={() => setChoice('high')} className={`p-6 rounded-xl border-2 transition-all ${choice === 'high' ? 'border-cyan-500 bg-cyan-500/10' : 'border-white/10 hover:border-white/20 bg-white/[0.02]'}`}>
          ...
          <AnimatePresence mode="wait">
            {isRolling ? (
              ...
              <motion.div key="result" initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
                ...
                <div className="text-white/40 mt-1">{result.number > 50 ? 'High' : 'Low'}</div>
