      ...
      {!isRolling && !result && (
        <div className="grid grid-cols-2 gap-3 mb-6">
          <button onClick={() => setChoice('low')} className={`choice-btn ${choice === 'low' ? 'choice-btn-selected' : ''}`}>
            <div className="text-2xl sm:text-3xl font-bold text-white mb-1">1-50</div>
          ...
          <button onClick={() => setChoice('high')} className={`choice-btn ${choice === 'high' ? 'choice-btn-selected' : ''}`}>
            <div className="text-2xl sm:text-3xl font-bold text-white mb-1">51-100</div>
        ...
        <AnimatePresence mode="wait">
          {isRolling ? (
            ...
            <motion.div key="wait" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center">
              ...
              <div className="text-sm text-white/30">Pick high or low</div>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
